...
describe Ashikawa::Core::Collection do
  ...
  describe "initialized connection" do
    ...
    it "should send a put request" do
      stub_request(:put, "http://localhost:8529/my/path").with(:body => {"name"=>"new_collection"}).to_return body: "{ \"name\": \"dude\" }"
      
      subject.send_request "/my/path", put: { :name => 'new_collection' }
      
      WebMock.should have_requested(:put, "http://localhost:8529/my/path").with :body => {"name"=>"new_collection"}
    end
    
    it "should send a delete request" do
      stub_request(:delete, "http://localhost:8529/my/path").to_return body: "{ \"name\": \"dude\" }"
      
      subject.send_request "/my/path", delete: { }
      
      WebMock.should have_requested(:delete, "http://localhost:8529/my/path")
    end
